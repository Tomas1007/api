...
public class ComentarioControlador {
    ...
    @GetMapping("/inmueble")
    public ResponseEntity<Page<ComentarioListDto>>listarComentario(@RequestParam(name = "id") Integer id,
                                                                   @RequestParam(value = "page",defaultValue = "0", required = false)int page,
                                                                   @RequestParam(value = "size",defaultValue = "10", required = false)int size){
        return ResponseEntity.ok(comentarioServicio.listarComentariosInmueble(id, page, size));
    }
